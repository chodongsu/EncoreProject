...
output_file_name = 'C:/Users/Playdata/Documents/moonju/csvfile/eml_parse_test.csv'
output_file = open(output_file_name, 'w', encoding='utf8', newline='')

# header를 dictionary로 변환했을때 key 값들을 미리 리스트로 지정해둠.
header_col_names= ['Received', 'Received-SPF', 'ARC-Seal', 'ARC-Message-Signature', 
'ARC-Authentication-Results', 'DKIM-Signature', 'From', 'To', 'CC', 
'Subject', 'Thread-Topic', 'Thread-Index', 'Importance', 'X-Priority', 'Date', 
'Message-ID', 'References', 'In-Reply-To', 'Accept-Language', 'Content-Language', 
'X-MS-Has-Attach', 'X-MS-TNEF-Correlator', 'x-incomingtopheadermarker', 'x-ms-exchange-messagesentrepresentingtype',
 'x-tmn', 'x-ms-publictraffictype', 'x-incomingheadercount', 'x-eopattributedmessage', 
 'x-ms-office365-filtering-correlation-id', 'x-ms-traffictypediagnostic', 'x-microsoft-antispam', 
 'x-microsoft-antispam-message-info', 'x-forefront-antispam-report', 'x-ms-exchange-antispam-messagedata', 
 'x-ms-exchange-transport-forked', 'X-OriginatorOrg', 'X-MS-Exchange-CrossTenant-RMS-PersistedConsumerOrg',
  'X-MS-Exchange-CrossTenant-Network-Message-Id', 'X-MS-Exchange-CrossTenant-rms-persistedconsumerorg', 
  'X-MS-Exchange-CrossTenant-originalarrivaltime', 'X-MS-Exchange-CrossTenant-fromentityheader', 
  'X-MS-Exchange-CrossTenant-id', 'X-MS-Exchange-Transport-CrossTenantHeadersStamped', 'X-TM-AS-GCONF', 
  'X-TM-AS-Product-Ver', 'X-TM-AS-Result', 'X-imss-scan-details', 'X-TM-AS-User-Approved-Sender', 
  'X-TM-AS-User-Blocked-Sender', 'X-TMASE-Version', 'X-TMASE-Result', 'X-TMASE-MatchedRID', 'X-TMASE-SNAP-Result', 
  'X-imss-reprocess-rules', 'X-imss-reprocess-type', 'MIME-Version', 'Content-Type']

main_col_names=['text_without_tag','text_with_tag','include_url','num_of_imgs']
csv_col_names = ['file_name'] + header_col_names + main_col_names
csvwriter = csv.writer(output_file)
csvwriter.writerow(csv_col_names)

# data를 불러올 경로 설정
path = 'C:/Users/Playdata/Documents/moonju/eml/eml2/'
...
def padding(data):
    data += '=' * (len(data) % 4)
    return data
    # data = re.sub(rb'[^a-zA-Z0-9%s]', rb'', data)
    # data += b'=' * (len(data) % 4)
...
for fle in listing:
    ...
    if str.lower(fle[-3:])=="eml":
        ...
        csv_list += [main_text_no_tag, main_text, include_url ,num_imgs]

        # 합쳐진 csv_list를 csv에 입력
        csvwriter.writerow(csv_list)

print(len(not_open_file))
print(not_open_file)